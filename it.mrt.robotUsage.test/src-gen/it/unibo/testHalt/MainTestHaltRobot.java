...
/*
 * ------------ SEMANTICS --------------------------------
 * A RobotSystem is composed of n>=0 cintexts and one Robot
...
public class MainTestHaltRobot extends ActorContext{
private USBTetherClientStreamer utUsb ;
private Invocable jsEngine;
private ScriptEngine engine;
private	int userCmd;
	public MainTestHaltRobot(String name, IOutputView outView,
	...
	public void configure() {
		try {
			...
			new EventSimulator(outView,"halt",5000).activate(SituatedSysKb.executorManyThread);
openNashorn();
 		} catch (Exception e) {
 			e.printStackTrace();
		} 		
...
protected void openNashorn() throws Exception{
	...
	do{
 		...
 		outView.addOutput("MainTestHaltRobot result "  + result );	
 		userCmd = waitForUserCommand( );
 		}while(userCmd != 'e' )	;
	outView.addOutput("bye bye"  );
	//	platform.raiseEvent("MainTestHaltRobot", EventPlatformKb.endOfJob, "mormalEnd");
	System.exit(1);
}
...
*/
	public static void main(String[] args) throws Exception{
		InputStream sysKbStream = new FileInputStream("testhalt.pl");
		InputStream sysRulesStream = new FileInputStream("sysRules.pl");
		new MainTestHaltRobot("testHaltRobot", EventPlatformKb.stdOutView, sysKbStream, sysRulesStream ).configure();
 	}
